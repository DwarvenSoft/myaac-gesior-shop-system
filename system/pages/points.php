...
*/
$config['paypal_active'] = true; // config is in './system/pay/paypal/'
$config['zaypay_active'] = true; // config is in './system/pay/zaypay/'
$config['contenidopago_active'] = true; // config is in './system/pay/contenidopago/'
/* POLISH SYSTEMS:
...
{
	#################################################################################
	$sms_type = isset($_POST['sms_type']) ? (int) $_POST['sms_type'] : NULL;
	$posted_code = isset($_POST['code']) ? $_POST['code'] : NULL;
	$to_user = isset($_POST['to_user']) ? $_POST['to_user'] : NULL;
	$errors = array();
	#################################################################################
	if(!empty($to_user))
	{
			...
			if(count($errors) == 0)
			{
				...
				else
				{
					if(add_points($account, $config['dotpay'][$sms_type]['addpoints']))
					{
						...
						echo '<h1><font color="red">Dodano '.$config['dotpay'][$sms_type]['addpoints'].' punktow premium do konta: '.htmlspecialchars($to_user).' !</font></h1>';
					}
					else
	...
	echo '<div style="background-color:gray; padding: 10px 10px 10px 10px">';
	if(count($errors) > 0)
	{
	...
	{
		echo '<h2>SMS</h2><span style="font-size:16px">Kup punkty premium, możesz je wymienić w sklepie OTSa na przedmioty w grze, aby zakupić punkty premium wyślij SMSa:</span><br />';
		foreach($config['dotpay'] as $sms)
			if($sms['type'] == 'sms')
				echo '<br /><span style="font-size:20px"><b>* Na numer <font color="darkred">'.$sms['sms_number'].'</font> o treści <font color="darkred"><b>'.$sms['sms_text'].'</b></font> za <font color="darkred"><b>'.$sms['sms_cost'].'</b></font>, a za kod dostaniesz <font color="darkred"><b>'.$sms['addpoints'].'</b></font> punktów premium.</b></span>';
		echo '<span style="font-size:16px"><br />W SMSie zwrotnym otrzymasz specjalny kod. Wpisz ten kod w formularzu wraz z NAZWĄ KONTA (nie nickiem postaci!) osoby która ma otrzymać punkty.<br />
	...
	echo '<tr><td><b>NAZWA KONTA: </b></td><td><input type="text" size="20" value="'.htmlspecialchars($to_user).'" name="to_user" /></td></tr>
...
elseif (isset($_REQUEST['system']) && $_REQUEST['system'] == 'paypal' && $config['paypal_active'])
{
	require(SYSTEM . 'pay/paypal/paypal.php');
}
elseif (isset($_REQUEST['system']) && $_REQUEST['system'] == 'homepay' && $config['homepay_active'])
{
	...
	if(!empty($to_user))
	{
		...
		if(count($errors) == 0)
		{
			...
			else
			{
				...
				{
					echo '<h1><font color="red">Dodano '.$config['homepay'][$sms_type]['addpoints'].' punktów premium do konta: '.$to_user.' !</font></h1>';
				}
				else
	...
	if($config['homepay_active_sms'])
	{
		...
		foreach($config['homepay'] as $typ)
			if($typ['type'] == 'sms')
				echo '<b>* Na numer <font color="green">'.$typ['sms_number'].'</font> o tresci <font color="green"><b>'.$typ['sms_text'].'</b></font> za <font color="green"><b>'.$typ['sms_cost'].'</b></font>, a za kod dostaniesz <font color="green"><b>'.$typ['addpoints'].'</b></font> punktów premium.</b><br/>';
		echo '</td></tr></table><br />';
	}
	if($config['homepay_active_transfer'])
	{
		...
		foreach($config['homepay'] as $typ)
			if($typ['type'] == 'przelew')
				echo '<b>* Adres - <a href="'.$typ['link'].'" target="_blank"><font color="green">'.$typ['link'].'</font></a> - koszt <font color="green"><b>'.$typ['przelew_cost'].'</b></font>, a za kod dostaniesz <font color="green"><b>'.$typ['addpoints'].'</b></font> punktów premium.</b><br/>';
		echo '</td></tr></table><br />';
	}
	echo '<table><tr><td><form action="?subtopic=points&system=homepay" method="POST"><table>';
	echo '<tr><td><b>Nazwa konta: </b></td><td><input type="text" size="20" value="'.htmlspecialchars($to_user).'" name="to_user" /></td></tr>
	<tr><td><b>Kod z SMSa: </b></td><td><input type="text" size="20" value="'.htmlspecialchars($posted_code).'" name="code" /></td></tr><tr><td><b>Typ wyslanego SMSa: </b></td><td><select name="sms_type">';
	...
	echo '</select></td></tr><tr><td></td><td><input type="submit" value="Sprawdz" /></td></tr></table></form>
	</td></tr></table><br />
	<table><tr><td>
...
elseif (isset($_REQUEST['system']) && $_REQUEST['system'] == 'zaypay' && $config['zaypay_active'])
{
	if($logged)
	{
		...
		echo '<span style=""><center><h1>Buy points by Zaypay</h1></center><br />Zaypay accepts SMSes and phone calls from many countries. Select how many points you want buy and check if your country is on list of accepted countries.<br />After payment you will receive points in 5-10 seconds.</span>';
		foreach($options as $option)
		{
			echo '<script src="http://www.zaypay.com/pay/' . $option['payalogue_id'] . '.js" type="text/javascript"></script>';
			echo '<br /><div style="width:100%;height:40px;background-color:#333333"><div style="float:left;width:50%;text-align:center;color:white"><h2>' . $option['name'] . ':</h2></div>';
			echo '<div style="float:right;height:40px;text-align:left"><a href="http://www.zaypay.com/pay/' . $option['payalogue_id'] . '?acc=' . $account_logged->getId() . '" onclick="ZPayment(this); return false" ><img src="http://www.zaypay.com/pay/' . $option['payalogue_id'] . '/img" border="0" style="margin-top:2px" /></a></div></div>';
		}
	}
	else
	{
...
elseif(isset($_REQUEST['system']) && $_REQUEST['system'] == 'contenidopago' && $config['contenidopago_active'])
{
	if($logged)
	{
		require_once(SYSTEM . 'pay/contenidopago/config.php');
			echo '<script src="http://promo.contenidopago.com/js/contenidopago.js" type="text/javascript"></script>	
...
else
{
	if($config['dotpay_active'])
		echo '<br /><br /><div style="background-color:gray;padding:20px 20px 20px 20px"><center><a href="?subtopic=points&system=dotpay"><h2>For Polish players - Dotpay.pl</h2><h3>Po co przepłacać? Kup punkty w promocyjnej cenie specjalnie dla polaków!</h3><h2>KLIKNIJ TU</h2></a></center></div>';
	if($config['homepay_active'])
		echo '<br /><br /><div style="background-color:gray;padding:20px 20px 20px 20px"><center><a href="?subtopic=points&system=homepay"><h2>For Polish players - Homepay.pl</h2><h3>Po co przepłacać? Kup punkty w promocyjnej cenie specjalnie dla polaków!</h3><h2>KLIKNIJ TU</h2></a></center></div>';
	if($config['paypal_active'])
		echo '<br /><br /><div style="background-color:gray;padding:20px 20px 20px 20px"><center><a href="?subtopic=paypal"><h2>PayPal</h2><h3>Cheapest points! Send us money from your PayPal account or credit card.</h3><h2>PRESS HERE!</h2></a></center></div>';
	if($config['zaypay_active'])
		echo '<br /><br /><div style="background-color:gray;padding:20px 20px 20px 20px"><center><a href="?subtopic=points&system=zaypay"><h2>ZayPay</h2><h3>Send us money using SMS or phone call.</h3><h2>PRESS HERE!</h2></a></center></div>';
	if($config['contenidopago_active'])
		echo '<br /><br /><div style="background-color:gray;padding:20px 20px 20px 20px"><center><a href="?subtopic=points&system=contenidopago"><h2>Contenidopago</h2><h3>Send us money using SMS or phone call.</h3><h2>PRESS HERE!</h2></a></center></div>';
}